...
export default function UploadZone({ onLogoClick }: UploadZoneProps) {
    ...
    const exportContent = useMemo(() => {
        ...
        return generateRepositoryContext(files, exportFormat, exportMode, exportIntent);
    ...
    const exportSizeBytes = useMemo(() => {
        if (files.length === 0 || !exportContent) return 0;
        return new Blob([exportContent]).size;
    }, [files, exportContent]);

    const formattedExportSize = useMemo(() => {
        if (exportSizeBytes === 0) return "0 KB";
        if (exportSizeBytes > 1024 * 1024) {
            return `${(exportSizeBytes / 1024 / 1024).toFixed(2)} MB`;
        }
        return `${(exportSizeBytes / 1024).toFixed(1)} KB`;
    }, [exportSizeBytes]);
    ...
    const handleDirectDownload = () => {
        if (!exportContent) return;
        const filename = `codemelt-context.${exportFormat === "markdown" ? "md" : "xml"}`;
    ...
    async function handleFilesUploaded(
        ...
        const rawFiles: RawFile[] = uploadedFiles.map((file) => ({
            name: file.name,
        ...
        const result = await scanFiles(rawFiles);
            ...
            <div className="mx-auto max-w-4xl w-full min-w-0">
                ...
                {files.length === 0 ? (
                    ...
                    <div className="space-y-8 animate-fadeIn">
                        ...
                        <RepositoryInsights analysis={repositoryAnalysis} exportSizeBytes={exportSizeBytes} />
